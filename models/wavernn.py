...
def save_wav(x, path):
    sf.write(path, x.astype(np.float32), samplerate=hp.dsp.sample_rate)
...
class WaveRNN(nn.Module):
    ...
    # @autocast(enabled=hp.voc_mixed_precision)
    ...
    def generate(self, mels, save_path: Union[str, Path], batched, target, overlap, mu_law):
        ...
        with torch.no_grad():
            ...
            mels = self.pad_tensor(mels, pad=self.pad, side='both')
            ...
            for i in range(seq_len):
                ...
                if self.mode == 'MOL':
                    ...
                    sampled = distrib.sample().float()
                    sample = 2 * sampled / (self.n_classes - 1.) - 1.
    ...
    def pad_tensor(self, x, pad, side='both'):
        ...
        padded = torch.zeros(b, int(total), c, device=x.device)
    ...
    def fold_with_overlap(self, x, target, overlap):
        ...
        num_folds = int((total_len - overlap) // (target + overlap))
        ...
        folded = torch.zeros(num_folds, int(target + 2 * overlap), int(features), device=x.device)

        # Get the values for the folded tensor
        for i in range(num_folds):
            start = int(i * (target + overlap))
            end = int(start + target + 2 * overlap)
    ...
    def xfade_and_unfold(self, y, target, overlap):
        ...
        total_len = int(num_folds * (target + overlap) + overlap)

        # Need some silence for the rnn warmup
        silence_len = int(overlap // 2)
        fade_len = int(overlap - silence_len)
        ...
        y[:, :int(overlap)] *= fade_in
        y[:, int(-overlap):] *= fade_out
